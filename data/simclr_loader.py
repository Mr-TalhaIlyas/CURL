...
import pywt
from data.augmentors import SimCLR_augment
...
class GEN_DATA_LISTS():
    ...
    def merge_paths(self, dict1, dict2):
        merged_data = {
            'flow_paths': dict1['flow_paths'] + dict2['flow_paths'],
            'vid_paths': dict1['vid_paths'] + dict2['vid_paths'],
            'lbls': dict1['lbls'] + dict2['lbls']
        }
        return merged_data
...
class SimCLR_FetalDataset(data.Dataset):
        ...
        for vid_path, flo_path, lbl_path in zip(self.video_paths, self.flow_paths, self.label_paths):
            ...
            for segment in segments:
                ...
                for clip in clips:
                    ...
                    samples.append((vid_path, flo_path, clip, segment[2], filename))  # all sub labels
    ...
    def _is_mostly_black(self, frames, threshold=0.6): # more than 60% pixels are black
    ...
    def __getitem__(self, idx):
        ...
        vr = decord.VideoReader(vid_path, width=self.config['video_width'], height=self.config['video_height'])
        # if not self.augment:
        ...
        # if not self.augment:
        #     ffrmes = fr.get_batch(adjusted_clip).asnumpy()
        try:
            timestamps = self.timestamps[adjusted_clip]
        except IndexError:
        ...
        if self.augment:
            vframes_i, vframes_j = SimCLR_augment(vframes)
            # as augments might return float values
            if vframes_i.max() <= 1:
                vframes_i = (vframes_i * 255).astype(np.uint8)
            if vframes_j.max() <= 1:
                vframes_j = (vframes_j * 255).astype(np.uint8)

            if self._is_mostly_black(vframes_i) or self._is_mostly_black(vframes_j):
                next_idx = (idx + 1) % len(self.samples)
                return self.__getitem__(next_idx)
            if self._is_solid_color(vframes_i) or self._is_solid_color(vframes_j):
                next_idx = (idx + 1) % len(self.samples)
                return self.__getitem__(next_idx)
            
            data_sample['vid_i'] = vframes_i
            data_sample['vid_j'] = vframes_j
            data_sample['lbl'] = -1
        else:
            if self._is_mostly_black(vframes):
                next_idx = (idx + 1) % len(self.samples)
                return self.__getitem__(next_idx)
            if self._is_solid_color(vframes):
                next_idx = (idx + 1) % len(self.samples)
                return self.__getitem__(next_idx)

            data_sample['vid'] = vframes
            # data_sample['flo'] = ffrmes
            data_sample['lbl'] = label
            data_sample['timestamp'] = (start_time, end_time)
